...
# so that we can conveniently check the effect of group label
track2.data_transform = lambda x: x * 0.1

# %%
# Let's create the first GV where no group label will be applied
gv1 = GenomeViewer()
...
# %%
# Let's create the second GV where we apply group label to the two tracks
gv2 = GenomeViewer()

gv2.add_track(track1)
gv2.add_track(track2)

gv2.add_group_label(0, 1, "Group 1")
